...
SUBDIRS=kafka-agent mirror-maker-agent tracing-agent crd-annotations test crd-generator api crd-convert mockkube certificate-manager operator-common config-model config-model-generator cluster-operator topic-operator user-operator kafka-init docker-images helm-charts/helm2 helm-charts/helm3 install examples
